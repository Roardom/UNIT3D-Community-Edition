...
@section('content')
    <x-panel heading="Queued Torrents">
        <table class="data-table">
            ...
            @foreach ($pending as $torrent)
                <tr>
                    ...
                    <td>
                        <i class="{{ $torrent->category->icon }}" title="{{ $torrent->category->name }} Torrent"></i>
                    </td>
                    ...
                    <td>
                        <x-chip.user
                            :anon="false"
                            :userId="$torrent->user->id"
                            :username="$torrent->user->username"
                            :href="route('users.show', ['username' => $torrent->user->username])"
                            :icon="$torrent->user->group->icon"
                            :color="$torrent->user->group->color"
                            :group="$torrent->user->group->name"
                        />
                    <td>
                        <form
                            method="POST"
                    ...
                    <td>
                        <button data-target="#pendpostpone-{{ $torrent->id }}" data-toggle="modal">
                            <i class="{{ config('other.font-awesome') }} fa-pause"></i>
                            {{ __('common.moderation-postpone') }}
                        </button>
                        <div class="modal fade" id="pendpostpone-{{ $torrent->id }}" tabindex="-1" role="dialog"
                             aria-hidden="true">
                    ...
                    <td>
                        <button data-target="#pendreject-{{ $torrent->id }}" data-toggle="modal">
                            <i class="{{ config('other.font-awesome') }} fa-thumbs-down"></i>
                            {{ __('common.moderation-reject') }}
                        </button>
                        <div class="modal fade" id="pendreject-{{ $torrent->id }}" tabindex="-1" role="dialog"
                             aria-hidden="true">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
                                                    <p id="title" name="title"
                                                       class="form-control-static">{{ $torrent->name }}</p>
                                                </div>
                                            </div>
                                            <div class="form-group">
    ...
    <x-panel :heading="__('torrent.postponed-torrents')">
        <table class="data-table">
            <thead>
            <tr>
            ...
            <tbody>
            @foreach ($postponed as $torrent)
                <tr>
                    <td>{{ $torrent->moderated_at->diffForHumans() }}</td>
                    <td>
                        <a href="{{ route('torrent', ['id' => $torrent->id]) }}">
                            {{ $torrent->name }}
                        </a>
                    ...
                    <td>
                        <x-chip.user
                            :anon="false"
                            :userId="$torrent->moderated->id"
                            :username="$torrent->moderated->username"
                            :href="route('users.show', ['username' => $torrent->moderated->username])"
                            :icon="$torrent->moderated->group->icon"
                            :color="$torrent->moderated->group->color"
                            :group="$torrent->moderated->group->name"
                        />
                    </td>
                    <td>
                        <form method="POST"  action="{{ route('staff.moderation.approve', ['id' => $torrent->id]) }}">
                            @csrf
                            <button type="submit">
                                <i class="{{ config('other.font-awesome') }} fa-thumbs-up"></i>
                                {{ __('common.moderation-approve') }}
                    ...
                    <td>
                        ...
                        <div class="modal fade" id="postdelete-{{ $torrent->id }}" tabindex="-1" role="dialog"
                             aria-hidden="true">
    ...
    <x-panel :heading="__('torrent.rejected')">
        <table class="data-table">
            <thead>
            <tr>
                <th>{{ __('staff.moderation-since') }}</th>
